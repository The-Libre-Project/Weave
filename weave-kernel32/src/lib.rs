...
const HEAP_SLAB_SIZE: usize = 8 * 1024 * 1024; // 8 MB per slab
    ...
    while scan_ptr < scan_limit && seen.len() < 64 {
        ...
        if fd_to_use >= 0 {
            let n = unsafe {
                libc::pread(
                    fd_to_use,
                    &mut candidate as *mut u64 as *mut libc::c_void,
                    8,
                    scan_ptr as i64,
                )
            };
...
pub unsafe extern "win64" fn enter_critical_section(lp_critical_section: *mut u8) {
    ...
    loop {
        match lock_atomic.compare_exchange(-1, 0, Ordering::AcqRel, Ordering::Acquire) {
            ...
            Err(curr) => {
                // Uninitialized CS: LockCount was never set to -1 (side-by-side
                // DLL's DllMain was not called — static CRT init never ran).
                // Force-acquire: CAS(0 → 0) does nothing and lets us set the
                // owning thread. LockCount stays 0, which is correct for a CS
...
fn heap_alloc_slab(alloc_size: usize, total_size: usize) -> *mut std::ffi::c_void {
    ...
    if slab_base == libc::MAP_FAILED {
        eprintln!("weave/HeapAlloc: slab mmap(MAP_32BIT) failed size={HEAP_SLAB_SIZE:#x}, retrying without MAP_32BIT");
        slab_base = unsafe {
            libc::mmap(
                0x0000_0000_0040_0000 as *mut libc::c_void,
                HEAP_SLAB_SIZE,
        ...
        if slab_base == libc::MAP_FAILED {
            eprintln!(
                "weave/HeapAlloc: slab mmap (no MAP_32BIT) also failed size={HEAP_SLAB_SIZE:#x}"
            );
            return std::ptr::null_mut();
        }
    }
...
pub unsafe extern "win64" fn get_file_attributes_ex_w(
    ...
    let attrs = if (stat.st_mode & libc::S_IFMT) == libc::S_IFDIR {
        ...
        FILE_ATTRIBUTE_NORMAL // 0x80 — real Windows returns 0x20 (ARCHIVE) for files
    };

    // Fill WIN32_FILE_ATTRIBUTE_DATA
    unsafe {
        (*lp_file_information).dw_file_attributes = attrs;
...
pub unsafe extern "win64" fn get_drive_type_w(lp_root_path_name: *const u16) -> u32 {
    ...
    // Read the path prefix to determine the drive letter (A-Z followed by :).
    let first_char = unsafe { *lp_root_path_name };
    let second_char = unsafe { *lp_root_path_name.add(1) };
