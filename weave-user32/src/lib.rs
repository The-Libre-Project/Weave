...
pub fn resolve(dll: &str, func: &str) -> Option<usize> {
    ...
    match func {
        ...
        "SetClassLongPtrW" => Some(
            set_class_long_ptr_w as unsafe extern "win64" fn(_, _, _) -> _ as *const () as usize,
        ),
        ...
        "GetMenuCheckMarkDimensions" => {
            Some(api::get_menu_check_mark_dimensions as *const () as usize)
        }
        "RealChildWindowFromPoint" => Some(
            api::real_child_window_from_point as unsafe extern "win64" fn(_, _) -> _ as *const ()
                as usize,
        ),
        ...
        "SubtractRect" => {
            Some(api::subtract_rect as unsafe extern "win64" fn(_, _, _) -> _ as *const () as usize)
        }
        "InvertRect" => {
            Some(api::invert_rect as unsafe extern "win64" fn(_, _) -> _ as *const () as usize)
        }
        "ShutdownBlockReasonCreate" => Some(
            api::shutdown_block_reason_create as unsafe extern "win64" fn(_, _) -> _ as *const ()
                as usize,
        ),
        "ShutdownBlockReasonDestroy" => {
            Some(api::shutdown_block_reason_destroy as *const () as usize)
        }
        _ => None,
    }
}
