    ...
    pub unsafe fn virtual_unwind(
        ...
        loop {
            ...
            if flags & (UNW_FLAG_EHANDLER | UNW_FLAG_UHANDLER) != 0 {
                ...
                // SAFETY: `handler_addr` = image_base + handler_rva is the absolute
                // virtual address of a language-specific exception handler (e.g.
                // `__CxxFrameHandler4`) inside the PE image. The PE is mapped as
                // executable. transmute<usize, ExceptionHandlerFn> reinterprets the
                // address as a function pointer — valid because ExceptionHandlerFn is
                // declared `unsafe extern "win64"`, matching the Windows x64 calling
                // convention used by all PE-compiled handlers. No Rust type is
                // misrepresented: usize and fn-pointer have the same size (8 bytes on
                // x86_64) and we never call it except through the Option wrapper.
                handler =
    ...
    pub unsafe extern "win64" fn cxx_frame_handler(
        ...
        for i in 0..n_try_blocks {
            ...
            for j in 0..n_catches {
                ...
                //
                // SAFETY: transmute of `handler_va` (a usize) to a Win64 fn pointer.
                // handler_va = image_base + handler_rva, where handler_rva is the
                // dispOfHandler field from the HandlerType — an RVA to the compiler-
                // generated catch funclet inside the PE's .text section. The PE is mapped
                // executable. The funclet signature `fn(u64, u64) -> u64` matches the
                // MSVC x64 EH ABI: funclets receive (exception_object_ptr, frame_ptr) in
                // rcx/rdx and return the continuation VA in rax. The `extern "win64"`
                // ABI ensures the Windows x64 calling convention is used.
