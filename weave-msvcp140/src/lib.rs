...
fn get_fake_locale_data() -> &'static FakeLocaleData {
    let data = FAKE_LOCALE_DATA.get_or_init(|| {
        ...
        let retfirst_thunk_addr = thunk_page as usize;
        let retzero_thunk_addr = thunk_page as usize + 16;
        // _Locimp vtable entries use the retfirst thunk
        data._pad_before_vtable = [retfirst_thunk_addr; 8];
        data.vtable = [retfirst_thunk_addr; 64];
        // Facet vtable entries use the retzero thunk
    ...
    LAYOUT_DIAG.get_or_init(|| {
        ...
        let li = &data.locimp_vtable as *const usize as usize;
        eprintln!("  _Locimp @ {li:#x}");
...
pub unsafe extern "win64" fn msvcp_tolower_range(
    _this: *const u8,
    dst: *mut u8,
    _src: *const u8,
    _d: usize,
...
pub unsafe extern "win64" fn msvcp_cnd_register_thread_exit(
    _cnd: *mut libc::c_void,
    _b: usize,
    _c: usize,
    _d: usize,
) {
}
pub unsafe extern "win64" fn msvcp_cnd_unregister_thread_exit(
    _b: usize,
    _c: usize,
    _d: usize,
    _e: usize,
...
pub fn resolve(dll: &str, func: &str) -> Option<usize> {
    ...
    let addr = match func {
        ...
        "?getline@?$basic_istream@DU?$char_traits@D@std@@@std@@QEAAAEAV12@PEAD_J@Z" => { msvcp_ostream_flush as unsafe extern "win64" fn(*mut u8,usize,usize,usize)->*mut u8 as *const () as usize }
        "?_Winerror_map@std@@YAHH@Z" => { msvcp_ios_eof as unsafe extern "win64" fn(usize,usize,usize,usize)->i32 as *const () as usize }
        "?_Xoverflow_error@std@@YAXPEBD@Z" => { msvcp_osfx_nop as unsafe extern "win64" fn(usize,usize,usize,usize) as *const () as usize }
        "?_Xregex_error@std@@YAXW4error_type@regex_constants@1@@Z" => { msvcp_osfx_nop as unsafe extern "win64" fn(usize,usize,usize,usize) as *const () as usize }
        "?_Xruntime_error@std@@YAXPEBD@Z" => { msvcp_osfx_nop as unsafe extern "win64" fn(usize,usize,usize,usize) as *const () as usize }
        "?eof@ios_base@std@@QEBA_NXZ" => { msvcp_ios_eof as unsafe extern "win64" fn(usize,usize,usize,usize)->i32 as *const () as usize }
        "?bad@ios_base@std@@QEBA_NXZ" => { msvcp_ios_bad as unsafe extern "win64" fn(usize,usize,usize,usize)->i32 as *const () as usize }
        "?gcount@?$basic_istream@DU?$char_traits@D@std@@@std@@QEBA_JXZ" => { msvcp_ios_width_get as unsafe extern "win64" fn(usize,usize,usize,usize)->i64 as *const () as usize }
        "?peek@?$basic_istream@DU?$char_traits@D@std@@@std@@QEAAHXZ" => { msvcp_underflow as unsafe extern "win64" fn(*const u8,usize,usize,usize)->i32 as *const () as usize }
        "?seekp@?$basic_ostream@DU?$char_traits@D@std@@@std@@QEAAAEAV12@V?$fpos@U_Mbstatet@@@2@@Z" => { msvcp_ostream_flush as unsafe extern "win64" fn(*mut u8,usize,usize,usize)->*mut u8 as *const () as usize }
