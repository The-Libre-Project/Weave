...
fn resolve_portaudio_ordinal_4() {
    ...
    assert!(
        addr.is_some(),
        "resolve(#4) must return Some for portaudio stub"
    );
...
fn resolve_portaudio_ordinal_70() {
    ...
    assert!(
        addr.is_some(),
        "resolve(#70) must return Some for portaudio stub"
    );
}
...
fn resolve_portaudio_ordinal_75() {
    ...
    assert!(
        addr.is_some(),
        "resolve(#75) must return Some for portaudio stub"
    );
}
...
fn resolve_portaudio_named_get_host_api_info() {
    ...
    assert!(
        addr.is_some(),
        "resolve(Pa_GetHostApiInfo) must return Some"
    );
}
